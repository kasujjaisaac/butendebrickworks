...
@section('admin-content')
    ...
    <div class="overflow-hidden rounded-sm border border-[#d8c0ad] bg-white shadow-[0_8px_24px_rgba(0,0,0,0.05)]">
        ...
        @else
            <form id="bulk-action-form" method="POST" action="{{ route('admin.products.bulk-delete') }}" onsubmit="return confirm('Delete selected products? This cannot be undone.')">
                @csrf
                <table class="w-full text-left text-sm">
                    ...
                    @foreach ($products as $product)
                        <tr class="transition hover:bg-[#fff7f2]">
                            ...
                            <td class="px-4 py-3">
                                <!-- Toggle active form (not nested in bulk form) -->
                                <form method="POST" action="{{ route('admin.products.toggle-active', $product) }}" style="display:inline">
                                    @csrf @method('PATCH')
                                    <button type="submit"
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <!-- Delete form (not nested in bulk form) -->
                    ...
                    @endforeach
                        <tr>
                            <td colspan="100%" class="px-4 py-3 bg-[#fff9f4] text-right">
                                <button type="submit" class="inline-flex items-center gap-1 rounded bg-red-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-red-700">
